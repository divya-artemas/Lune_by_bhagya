...
/*************** wp_nav_menu *****************************/
register_nav_menus( array('header' => __('Header Navigation', 'luna')));

/*************** end wp_nav_menu **************************/

/*************** wp_nav_menu footer *****************************/
register_nav_menus( array('footer' => __('Footer Navigation', 'luna')));
/*************** end wp_nav_menu footer **************************/

/*************** wp_nav_menu footer *****************************/
register_nav_menus( array('top_right' => __('Top Menu Navigation', 'luna')));
/*************** end wp_nav_menu footer **************************/


/*************** Theme Scripts   **************************/
function luna_scripts_styles() {
  /*
...
add_action( 'wp_enqueue_scripts', 'luna_scripts_styles' );
/*************** end Theme Scripts ************************/
...
function custom_body_class( $classes ) {
  ...
  elseif(is_page_template('template-contact.php')) {
    $classes[] = 'inner contact-page';
  }
  //elseif(is_page_template('template-blog.php') || is_single('blogs')) {
   // $classes[] = 'inner styling';
 // }
  elseif(is_page_template('template-faq.php') || is_page_template('template-stylingcontact.php')) {
    $classes[] = 'inner outfit-collages questions';
  }
  elseif(is_page_template('template-packages.php')) {
    $classes[] = 'inner package';
  } 
  elseif(is_page_template('template-outfit.php')) {
    $classes[] = 'inner outfit-collages';
  } 
  elseif(is_singular('outfit')){
    $classes[] = 'inner outfit-detailed';
  }
...
class themeslug_walker_nav_menu extends Walker_Nav_Menu {
 ...
 function lunat_lvl( &$output, $depth = 0, $args = array() ) {
 ...
 // add main/sub classes to li's and links
  function lunat_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
     global $wp_query;
     ...
     // build html
     $output .= apply_filters( 'walker_nav_menu_lunat_el', $item_output, $item, $depth, $args );
 }
 }
 /**************End walker_nav_menu ****************************/   
...
function cw_post_type_outfit() {
  ...
  $labels = array(
  'name' => _x('Outfits', 'plural'),
  'singular_name' => _x('Outfit', 'singular'),
  'menu_name' => _x('Outfits', 'admin menu'),
  'name_admin_bar' => _x('Outfits', 'admin bar'),
  'add_new' => _x('Add New', 'add new'),
  'add_new_item' => __('Add New Outfit'),
  'new_item' => __('New Outfit'),
  'edit_item' => __('Edit Outfit'),
  'view_item' => __('View Outfit'),
  'all_items' => __('All Oufits'),
  'search_items' => __('Search Outfits'),
  'not_found' => __('No outfits found.'),
  );
  $args = array(
  'supports' => $supports,
  'labels' => $labels,
  ...
  add_action('init', 'cw_post_type_outfit');
  /*Custom Post type end*/
